...
This document defines how AI coding agents should contribute to the current project. The goal is controlled, architecture-aligned execution that preserves business rules and system integrity.

## AI Operating Principles

- Treat `DOC/` as the authoritative source before making implementation decisions.
- Start with `DOC/framework/guide/ai/ai-context.yaml` when you need a compact system map or a task-based read path.
...
1. Read `DOC/framework/guide/ai/ai-context.yaml` first.
...
- required tests
- starting reference: `DOC/framework/guide/ai/ai-context.yaml`

Example:

"Start with DOC/framework/guide/ai/ai-context.yaml. Implement the admin assignment workflow for queue management. Use the queue rules in DOC/framework/guide, preserve admin-only assignment, create the project record transactionally, and add tests for duplicate assignment prevention."
