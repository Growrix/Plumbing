...
This document is a reusable product requirements baseline for role-based web applications. Replace or trim any sample modules that do not apply to the current project before using it as a delivery contract.

## Product Vision

Define the current project's value proposition, trust model, monetization, and support model here. The structure below fits service, content, and commerce-oriented products with public and authenticated surfaces, but it should be adapted per project instead of inherited unchanged.
...
- Interacts with the public AI assistant for guidance.
...
- Admin-only login from `/admin`.
- Developer-only login from `/team`.
...
- The AI assistant supports discovery, guidance, and dashboard assistance.
