---
name: enterprise-testing-quality-enforcer
description: Enterprise-grade quality enforcement for implementation tasks. Use when you need a mandatory build-validate-test-verify-optimize flow, blocking test gates, UI and mobile validation, SEO and accessibility checks, performance/security/regression validation, and strict stop-on-failure behavior. Trigger phrases include: enterprise testing, quality gate, full test pipeline, validate mobile, SEO check, accessibility check, security validation, regression testing, release readiness.
...
## Tool Discipline
- Use Read and Grep to gather required context first.
- Use Edit/Write for minimal targeted changes.
- Use Bash for test/build/validation commands.
- Use TodoWrite for multi-step enforcement workflows.
